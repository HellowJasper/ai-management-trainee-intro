...
现有服务入口是 `server/index.js`，当前已经提供静态资源服务、API 路由和若干 JSON repository。它已经不只是纯静态页面，组队、投票、评分草稿、作品提交、作品审核、审计日志和本地开发登录已经具备 JSON 级持久化能力。严格后端权限校验已经可以通过 `AUTH_ENFORCEMENT=strict` 开启。飞书 OAuth 后端 start / callback 骨架和 Redis Session repository 已经具备，但真实飞书应用凭证、正式数据库生产配置、真实 Redis 服务联调和实时推送还没有正式落地。
...
- `package.json` 当前脚本非常轻量：
  ...
  - `npm run dev:api:mysql:strict:redis` 使用 MySQL、后端强鉴权和 Redis Session 启动 API。
...
- 已有 repository 雏形：
  ...
  - `server/redisAuthSessionRepository.js`
- 已有数据库化基础：
...
- 飞书 OAuth start / callback 接口已经具备后端骨架，支持 state 校验、授权码换用户信息、按 `userId` / `openId` / `unionId` 匹配后端角色并创建 `feishu-oauth` session；真实飞书应用凭证和线上回调尚未联调。
- 飞书本地开发登录仍支持传入角色创建 `local-dev` session，也支持按后端角色映射创建 `role-mapping` session，并通过 HTTP-only Cookie 让 `/api/me` 读取登录态。
- 移动端的角色切换、组队、投票、评分草稿仍大量依赖 localStorage。
- 管理后台接口在 `AUTH_ENFORCEMENT=strict` 下可以做角色校验，但真实管理员身份仍依赖后续飞书 OAuth 接入。
...
- 管理后台已经有控制界面；严格模式下后台写接口可以做角色校验，但真实管理员身份仍依赖后续飞书 OAuth 接入。结果快照已经有基础发布能力，完整审计覆盖仍需继续补齐。
...
- 当前 JSON 文件可以通过 `npm run db:seed` 作为 seed 数据导入数据库；只更新用户与角色时使用 `npm run db:seed:users`，避免影响队伍、作品、投票、倒计时等比赛状态。
...
- `GET /api/auth/feishu/start` 和 `GET /api/auth/feishu/callback` 已存在，支持 OAuth state 校验、授权码换用户信息、角色映射和 Cookie Session 创建。
- `POST /api/auth/feishu/login` 已存在，继续支持本地角色登录和后端角色映射登录，用于未配置飞书凭证时联调。
- `server/feishuOAuthProvider.js` 已封装飞书 OAuth provider，默认从 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`、`FEISHU_REDIRECT_URI` 读取配置。
- `server/oauthStateRepository.js` 已提供 JSON 级一次性 OAuth state 存储。

下一步：

- 配置真实飞书应用并完成线上回调联调。
- 将 OAuth state 从 JSON 文件迁移到 Redis；Session 已具备 `SESSION_BACKEND=redis` 切换能力，仍需接真实 Redis 服务联调。
...
- Redis Session 真实服务联调、飞书 OAuth 线上联调、结果撤回策略和正式并发锁仍未落地。
...
- 已有飞书 OAuth start / callback 后端骨架，支持 state 校验、用户信息换取和角色映射 session 创建。
...
- 配置真实飞书应用并完成 OAuth 线上联调。
- 后端根据账号返回角色。当前本地映射和 OAuth 骨架已可用，仍需用真实飞书应用验证 open_id / union_id。
- 未绑定用户进入无权限状态。
- 后端成为权限最终判断方。
...
本节用于同步给后续一起开发后端的同事，重点说明当前已经做了什么、哪些还没有做，以及下一步应该从哪里接手。
...
- 已保留现有前端，不做前端框架重构。
- 已支持前后端分离本地运行：
  - `npm run dev:api`：启动后端 API，默认端口 `63779`。
  - `npm run dev:web`：启动前端静态服务，默认端口 `5174`，通过 `runtime-config.js` 指向 API。
  - `npm run dev:api:strict`：启动后端 API，并开启后端强鉴权。
  - `npm run dev:api:mysql`：使用 MySQL repository 启动 API。
  - `npm run dev:api:mysql:strict`：使用 MySQL repository，并开启后端强鉴权。
  - `npm run dev:api:mysql:strict:redis`：使用 MySQL repository、后端强鉴权和 Redis Session。
- 已支持跨端口 Cookie 请求和开发环境 CORS，前端 API 请求默认携带 Cookie。

数据层与数据库：

- 已建立 `db/schema.mysql.sql`，覆盖用户、角色、星锐资料、队伍、成员、作品、投票、评分、阶段、大屏状态、倒计时、路演、结果快照和审计日志核心表。
- 已建立 `server/mysqlClient.js`，通过 mysql2 创建数据库连接池。
- 已建立 `server/repositoryFactory.js`，支持 `DATA_BACKEND=json|mysql` 切换。
- 已完成下列 MySQL repository：
  - `server/mysqlTraineeRepository.js`
  - `server/mysqlTeamRepository.js`
  - `server/mysqlVoteResultsRepository.js`
  - `server/mysqlJudgeScoresRepository.js`
  - `server/mysqlWorksRepository.js`
  - `server/mysqlAuditLogRepository.js`
  - `server/mysqlMissionCountdownRepository.js`
  - `server/mysqlRoadshowRepository.js`
  - `server/mysqlAdminStateRepository.js`
  - `server/mysqlResultSnapshotsRepository.js`
  - `server/mysqlUserRoleRepository.js`
- 已建立 `server/mysqlSeedImporter.js`，可通过 `npm run db:seed` 把当前 `data/*.json` 演示数据导入 MySQL。

API 与业务能力：

- 已有健康检查、当前用户、权限查询、本地登录、退出登录 API。
- 已有用户角色管理 API：
  - `GET /api/admin/users`
  - `POST|PATCH /api/admin/users`
- 已有星锐资料 CRUD 和主持词保存 API。
- 已有组队、退出队伍、岗位职责认领 API。
- 已有大众投票、取消投票、投票窗口控制和投票排名读取 API。
- 已有专家评分草稿保存和评分状态读取 API。
- 已有作品提交、作品列表、作品详情和管理员审核状态 API。
- 已有阶段切换、时间显示、任务倒计时、路演计时、最终结果发布、最新结果快照读取和审计日志 API。

权限与登录：

- 已有 JSON 和 MySQL 两套用户角色映射 repository。
- 已有 `GET /api/auth/feishu/start` 和 `GET /api/auth/feishu/callback`，可完成 OAuth state 校验、授权码换用户信息、后端角色映射和 `feishu-oauth` session 创建。
- 已有 `server/redisAuthSessionRepository.js`，可通过 `SESSION_BACKEND=redis`、`REDIS_URL`、`SESSION_TTL_SECONDS`、`SESSION_REDIS_PREFIX` 切换到 Redis Session。
- `POST /api/auth/feishu/login` 当前支持两种过渡登录方式：
  - 传入 `role` 创建 `local-dev` 本地开发 session。
  - 不传 `role` 时，按 `userId` / `openId` / `unionId` 从后端用户角色映射创建 `role-mapping` session。
- `/api/me` 已能从 HTTP-only Cookie Session 返回用户、角色和权限。
- `AUTH_ENFORCEMENT=strict` 已接入服务启动流程，开启后组队、投票、评分、作品提交和管理写接口都会以后端 session 为准，不信任请求体里的身份字段。

测试：

- 当前已补充 Node test 覆盖核心逻辑、API、前后端分离、严格鉴权、MySQL repository 和 seed importer。
- 最近一次本地验证命令：`npm test`。
- 最近一次验证结果：`168/168` 通过。
...
- 尚未配置真实飞书应用并完成 OAuth / SSO 线上回调联调。
- OAuth state、回调、换 token、获取 open_id / union_id、绑定平台用户的后端代码骨架已经完成，但仍需要用真实飞书租户验证字段和权限范围。
- Session 默认仍使用 JSON 文件，便于本地开发；Redis Session repository 和启动开关已完成，但真实 Redis 服务仍需联调。
- Cookie 的生产配置仍需根据正式域名确认 `Secure`、`SameSite`、跨域策略和 HTTPS 反向代理配置。

数据库与并发：

- MySQL repository 已具备基础读写，但核心写操作还没有统一事务封装。
- 组队抢赛道、岗位认领、投票、评分提交等高并发场景还需要补事务、行锁或短期分布式锁。
- 投票和评分还没有最终提交锁定、撤回策略、重复提交幂等 key。
- 结果快照已能发布，但结果撤回、重新发布版本策略、正式公示锁定规则还没有完成。

后台能力：

- 管理后台已有基础控制页面，但并非所有后台规划能力都已接真实 API。
- 管理员身份最终仍依赖真实飞书登录；在此之前只能用本地开发 session 或角色映射联调。
- 审计日志已经覆盖部分关键写操作，但还没有做到所有管理员操作全覆盖。
- 大屏控制仍以接口读取 / 轮询为主，尚未接 WebSocket 或 Socket.IO 实时推送。

文件与作品：

- 作品提交目前以 URL 和 JSON 元数据为主。
- 尚未接企业对象存储或正式文件上传。
- 封面、附件、演示文件、截图等资源的权限、大小限制、病毒扫描和 CDN 策略还没有做。

前端正式切换：

- 前端仍保留 localStorage 和 JSON fallback，适合演示和弱网兜底，但正式业务状态应以后端 API 为准。
- 移动端部分角色切换、组队、投票、评分草稿逻辑仍有本地兜底，需要逐步改成以后端状态为准。
- 仍需要补完整 API 字段文档、错误码规则和前端调用约定。

部署：

- 尚未完成正式部署脚本、Nginx 配置、HTTPS、环境变量模板、CI/CD、数据库备份和回滚方案。
- 尚未完成现场压测和多设备联调。
...
```bash
npm run dev:api:strict
npm run dev:web
```

检查点：

- 未登录时，组队、投票、评分、作品提交、管理员写接口应返回 `401`。
- 错误角色访问对应写接口应返回 `403`。
- 登录后 `/api/me` 应返回后端 session 中的真实角色和权限。
...
npm run db:schema
npm run db:seed
npm run dev:api:mysql:strict
npm run dev:web
```

检查点：

- `/api/health` 返回 `runtime.dataBackend = "mysql"`。
- `/api/trainees`、`/api/teams`、`/api/vote-results`、`/api/admin/state` 能从数据库读取数据。
- 管理后台写操作能落库，并写入审计日志。
...
- 新增飞书 OAuth 配置：
  ...
  - `SESSION_SECRET`
  - `REDIS_URL`
- 联调现有 OAuth start / callback 流程。
- 回调拿到 `open_id` / `union_id` 后，已复用 `userRoleRepository.resolveLoginUser()` 获取平台角色；下一步需要用真实飞书账号验证。
- 未绑定用户不自动降级为大众评委，应返回无权限或待管理员绑定状态。

第五步：补核心写操作事务和并发控制。

- 组队抢赛道和岗位认领需要事务或行锁。
- 投票需要保证一人一票、窗口关闭后不可投、重复提交幂等。
- 专家评分需要区分草稿、最终提交、锁定。
- 结果发布需要版本策略和撤回策略。

### 13.4 当前协作边界

- 前端暂不重构，后端同事优先沿用现有 REST API 路径。
- 不要把 `src/site.js` 里未确认的占位入口当作正式需求。
- 不要新增 AI 应用层、向量检索或复杂数据平台；当前 MVP 不需要。
- 新增后端能力时优先补测试，至少覆盖 API 契约、权限边界和 MySQL repository 行为。
- 正式环境上线前，必须用 `AUTH_ENFORCEMENT=strict` 和 `DATA_BACKEND=mysql` 验收。
...
- `tests/logic.test.js`
- `tests/server.test.js`
- `tests/site-css.test.js`
- `tests/mysqlRepository.test.js`
- `tests/mysqlTeamRepository.test.js`
- `tests/mysqlVoteResultsRepository.test.js`
- `tests/mysqlJudgeScoresRepository.test.js`
- `tests/mysqlWorksRepository.test.js`
- `tests/mysqlAuditLogRepository.test.js`
- `tests/mysqlTimerRepositories.test.js`
- `tests/mysqlAdminStateRepository.test.js`
- `tests/mysqlResultSnapshotsRepository.test.js`
- `tests/mysqlUserRoleRepository.test.js`
- `tests/mysqlSeedImporter.test.js`
- `tests/redisAuthSessionRepository.test.js`

需要继续补充的测试：

- 真实飞书 OAuth 租户联调、权限 scope 和异常回调场景。
- 真实 Redis 服务连接、断线重连和过期策略联调。
- MySQL 事务回滚。
- 并发抢赛道、并发认领岗位职责。
- 并发投票下的一人一票。
- 专家评分最终提交、锁定和重复提交。
- 结果快照撤回、重新发布和版本保留。
- 正式部署环境下的 Cookie `Secure` / `SameSite` 策略。
