...
如果飞书/企业应用后台扩大了可访问用户范围，系统不会自动把新范围内的所有人实时拉进“用户管理”。原因是企业应用范围只控制“谁可以授权登录应用”，后台“用户管理”读取的是本系统自己的 `users` / `role_assignments` 数据。

推荐流程：

1. 从飞书通讯录、HR 系统或管理员已有名单导出完整 CSV。必须至少包含 `user_id`，建议包含 `name`、`department`、`role`。
2. 先执行 dry-run，只看新增、更新、重复 ID、缺失 ID 和非法角色，不写数据库：

   ```bash
   npm run db:import:users:csv -- /path/to/users.csv --dry-run
   ```

3. dry-run 没有问题后再执行正式导入：

   ```bash
   npm run db:import:users:csv -- /path/to/users.csv --execute --source=csv-20260703
   ```

4. 回到“用户管理”检查总人数、姓名、用户 ID、部门和角色。
5. 如果只是让新增范围用户自己登录，不需要提前出现在后台；用户首次飞书登录时，后端会写入用户身份，但不会自动派发选手/评委/管理员角色。

CSV 字段兼容：

- 用户 ID：`用户ID`、`user_id`、`userid`、`飞书用户ID`、`账号`、`工号`。
- 姓名：`姓名`、`name`、`display_name`。
- 部门：`部门`、`department`、`部门路径`。
- 角色：`角色`、`role`、`roles`，支持 `管理员/admin`、`评委/judge`、`选手/player`、`观众/public`。

角色规则：

- 如果 CSV 某一行没有填写角色，该用户会补 `public`，但不会清掉已有的管理员、评委或选手权限。
- 如果 CSV 某一行明确填写了角色，系统会把这个用户原来的 active 角色禁用，再写入 CSV 中的角色。
- 执行 `--execute` 前会备份 `users` 和 `role_assignments` 到 `docs/user-import-backups/`。

注意：不要为了同步用户去跑全量 seed，避免误触队伍、作品、投票、倒计时等比赛状态。CSV 用户导入只写 `users` 与 `role_assignments`，不写队伍、作品、投票、倒计时、评分表。
