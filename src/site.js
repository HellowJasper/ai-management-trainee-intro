...
(function attachSite(root, doc) {
  ...
  async function syncRoleFromBackend() {
    if (currentRole()) {
      ...
      try {
        ...
        if (me && me.user) {
          const singleRole = Array.isArray(me.roles) && me.roles.length === 1 ? me.roles[0] : "";
          const backendRole = VALID_ROLES.includes(me.role) ? me.role : (VALID_ROLES.includes(singleRole) ? singleRole : "");
          if (backendRole) {
            const session = { ...me, role: backendRole };
            storeSession(session);
            if (backendRole !== currentRole()) setRole(backendRole, session);
          } else {
            root.localStorage.removeItem(ROLE_KEY);
            storeSession(me);
            if (me.needsRoleSelection) showRolePicker(me.roles || []);
          }
          refreshRoleChrome();
        } else {
          clearStoredRole();
  ...
  function renderGallery() {
    ...
    return `${pageHead("作品展示大厅", "五支队伍 · 五大赛道 · 真实可运行作品", "GALLERY")}${dataNotice}${banner}<section class="container sec"><div class="gl2-grid horizontal">${cards}</div></section>`;
  }
