...
(function attachScreen(root, doc) {
  ...
  function renderBrief() {
    ...
    return `<div class="brief-wrap">${head("OVERVIEW")}<div class="brief-title-row"><div class="brief-title-block"><div class="title-frame"><h1 class="brief-h1">大赛介绍<span class="h1-dim"> 与 </span>全流程</h1></div><p class="brief-slogan">${esc(m.slogan)}</p></div><div class="brief-stats"><div class="stat"><b>${D.stats.teams}</b><span>参赛队伍 TEAMS</span></div><div class="stat"><b>${D.stats.members}</b><span>参赛者 MEMBERS</span></div><div class="stat"><b>${D.stats.tracks}</b><span>赛道 TRACKS</span></div><div class="stat"><b>${D.stats.mentors}</b><span>技术顾问 MENTORS</span></div></div></div><div class="tk-ribbon">${ribbon}</div><div class="brief-main"><section class="flow-panel">${cap("三天 · 全流程", "FULL JOURNEY")}<div class="day-track">${days}</div></section><section class="mech-panel">${cap("赛事机制", "RULES")}<div class="mech-grid">${mech}</div></section></div><footer class="brief-timeline"><div class="tl-head"><span class="live-dot"></span>LIVE 实时赛程</div><ul class="tl-track">${timeline}</ul><div class="tl-qr"><div class="qr-box">${qrSvg()}</div><div class="qr-text"><strong>手机端入口</strong><span>扫码查看赛程 · 投票</span></div></div></footer></div>`;
  }
  ...
  function renderTracks() {
    const cards = D.tracks.map((t, i) =>
      `<article class="track-card glass" style="--accent:${t.accent};--rgb:${t.rgb};--d:${i}"><span class="corner tl"></span><span class="corner br"></span><div class="track-head"><span class="track-code">${esc(t.code)}</span>${hud(t.icon, t.accent, 66)}</div><h3 class="track-name">${esc(t.name)}</h3><span class="track-en">${esc(t.en)}</span><div class="track-focus-wrap"><span class="track-flabel"><i class="cap-gt">&gt;</i>业务方向</span><p class="track-focus">${esc(t.focus)}</p></div><span class="track-flabel"><i class="cap-gt">&gt;</i>痛点关键词</span><div class="track-pains">${t.pains.map((p) => `<span>${esc(p)}</span>`).join("")}</div><div class="track-foot"><span class="track-mentor">顾问 · ${esc(t.mentor)}</span><a class="track-link" href="${esc(t.doc)}" target="_blank" rel="noopener">赛道讲解文档 ➔</a></div></article>`
    ).join("");
  ...
  function renderTeaming() {
    ...
    const cards = D.teams.map((t, i) => {
      ...
      return `<article class="tm-card glass" style="--accent:${t.accent};--rgb:${t.rgb};--d:${i}"><span class="corner tl"></span><span class="corner br"></span><div class="tm-head"><span class="track-code">${esc(t.trackCode)}</span><div class="tm-track"><b>${esc(t.track)}</b><span class="lane-en">TRACK</span></div><span class="tm-count">${cnt}<i>/${t.capacity}</i></span></div><div class="tm-advisor"><div class="tm-ava-wrap">${avatar(t.advisor, 108, "ring")}<span class="tm-ava-tag">技术顾问</span></div><b>${esc(t.advisor.name)}</b><span class="tm-role">ADVISOR · 赛道顾问</span></div><div class="tm-mcap"><i class="cap-gt">&gt;</i>组员实时加入 · MEMBERS</div><div class="tm-members">${memberStack(t, 56)}</div><div class="tm-bar">${bar((cnt / t.capacity) * 100, t.accent, t.rgb)}<span class="${locked ? "lk" : ""}">${locked ? "已满员锁定 🔒" : "招募中 " + cnt + "/" + t.capacity}</span></div></article>`;
    }).join("");
